...
class JiraClient:
    ...
    def get_boards_for_project(self, project_key: str) -> List[Dict[str, Any]]:
        ...
        Args:
            project_key: Jira project key (e.g., "RESMYB")

        Returns:
            List of board dictionaries with 'id', 'name', 'type'
        """
        try:
            # Use Jira Agile REST API to get boards for project
            ...
            for board in boards:
                result.append({
                    'id': board.id,
                    'name': board.name,
                    'type': getattr(board, 'type', 'unknown')
                })

            logger.info(f"Found {len(result)} boards for project {project_key}")
            return result
    ...
    def find_board_by_team_name(self, project_key: str, team_name: str) -> Optional[int]:
        """
        Find board ID by searching for team name in board names.

        Args:
            project_key: Jira project key (e.g., "RESMYB")
            team_name: Team name to search for (e.g., "Thunder")

        Returns:
            Board ID if found, None otherwise
        """
        boards = self.get_boards_for_project(project_key)

        if not boards:
            logger.warning(f"No boards found for project {project_key}")
            return None

        # Search for team name in board names (case-insensitive)
        team_lower = team_name.lower()

        for board in boards:
            board_name_lower = board['name'].lower()
            if team_lower in board_name_lower:
                logger.info(f"Found board '{board['name']}' (ID: {board['id']}) for team '{team_name}'")
                return board['id']

        # If no exact match, log available boards for debugging
        logger.warning(f"No board found containing '{team_name}' in name.")
        logger.info(f"Available boards for {project_key}:")
        for board in boards:
            logger.info(f"  - {board['name']} (ID: {board['id']})")

        return None

    def resolve_board_id(self, board_id: Optional[int] = None, project_key: Optional[str] = None, team_name: Optional[str] = None) -> Optional[int]:
        """
        Resolve board ID from various inputs.

        Priority:
        1. Direct board_id if provided
        2. Auto-detect from project_key + team_name
        3. Fall back to config

        Args:
            board_id: Direct board ID (highest priority)
            project_key: Project key for auto-detection
            team_name: Team name for auto-detection

        Returns:
            Resolved board ID or None
        """
        # Priority 1: Direct board_id
        if board_id:
            return board_id

        # Priority 2: Auto-detect from project_key + team_name
        if project_key and team_name:
            detected_id = self.find_board_by_team_name(project_key, team_name)
            if detected_id:
                return detected_id

        # Priority 3: Fall back to config
        return self.jira_config.get('board_id')
    ...
    def get_velocity(self, board_id: Optional[int] = None, num_sprints: int = 5) -> List[Dict[str, Any]]:
        ...
        """
        board_id = board_id or self.jira_config.get('board_id')
        velocity_data = []

        if not board_id:
            logger.warning("No board_id configured for velocity calculation")
        ...
        try:
            ...
            for sprint in sprints:
                ...
                for issue in issues:
                    sp_value = self._get_story_points_from_issue(issue)
                    completed_points += sp_value
    ...
    def _get_story_points_from_issue(self, issue) -> float:
        """Safely extract story points from an issue"""
        fields = issue.fields

        # List of common story point field names
        sp_field_candidates = [
            self.story_point_field,
            'customfield_10002',
            'customfield_10004',
